...
const offlineData: OfflinePromptData = {
  subjects: [
    "A strikingly handsome 35-year-old Pakistani man with perfectly styled natural jet-black hair, warm amber-brown eyes, authentic South Asian facial features, and a confident glowy expression",
    "A 36-year-old Pakistani gentleman with a well-groomed dark beard, warm wheatish complexion, realistic bone structure, and clearly natural facial proportions",
    "A refined 37-year-old Pakistani man with naturally textured black hair, defined masculine jawline, and a warm genuine smile free of distortion",
    "A dignified 38-year-old South Asian male with healthy glowing skin, expressive brown eyes, strong authentic proportions, and vivid photorealistic facial detail",
    "A confident 39-year-old Pakistani man with natural skin texture, subtle beard stubble, bright engaged eyes, and anatomically correct facial structure",
    "A polished 40-year-old Pakistani man with rich warm complexion, realistic pores, naturally defined jawline, and pleasant expressive eyes",
    "A handsome 35-year-old Pakistani man in a relaxed pose with glowing light skin tone, authentic South Asian bone structure, and no face distortion",
    "A charismatic 36-year-old Pakistani gentleman with deep brown eyes, natural eyebrow texture, and a warm approachable energy in a bright setting",
    "A professional 37-year-old South Asian man with healthy vibrant skin, subtle natural imperfections, and clearly realistic facial anatomy",
    "A cultured 38-year-old Pakistani man with full natural beard, warm complexion, and ultra-clear facial detail in bright lighting",
    "A confident 39-year-old Pakistani businessman with modern grooming, true-to-life skin texture, and a natural open expression",
    "A relaxed 40-year-old South Asian man with natural hair, warm brown eyes, and authentic proportioned facial features under glowing light",
    "A stylish 35-year-old Pakistani man with calm authoritative presence, real skin detail, and no artificial facial artifacts",
    "A quietly powerful 36-year-old South Asian gentleman with honest realistic anatomy, polished expression, and warm luminous complexion",
    "A naturally elegant 37-year-old Pakistani man with refined facial structure, bright eyes, and a vivid authentic portrait quality",
    "A warm 38-year-old South Asian man with bright glowing skin, realistically textured beard, and natural human proportions",
    "A focused 39-year-old Pakistani man in a glowing daytime environment with perfectly placed facial features and no distortion",
    "A softly smiling 40-year-old South Asian man with rich golden skin highlights, realistic pores, and precise facial anatomy",
    "A natural 35-year-old Pakistani man with genuine expressive eyes, strong jaw definition, and a light-filled radiant presence",
    "A mature 36-year-old South Asian gentleman with clear facial geometry, authentic skin tone, and the unmistakable look of a real human portrait",
  ],
  environments: [
    // Urban environments
  ...
  clothingStyles: [
    "a tailored linen kurta with minimal embroidery and breathable natural texture",
    "a modern tailored blazer over a crisp white shirt with subtle South Asian styling",
    "a classic charcoal suit with relaxed contemporary cut and premium fabric",
    "a smart casual cotton shirt paired with dark denim and polished leather loafers",
    "a traditional shalwar kameez in light neutral tones with elegant simplicity",
    "a sleek leather jacket layered over a refined neutral knit shirt",
    "a premium wool shawl draped over one shoulder with understated cultural grace",
    "a lightweight summer kurta in warm pastel shades with soft natural drape",
    "a crisp white dress shirt with rolled sleeves and minimalist tailored trousers",
    "a subtle patterned waistcoat paired with a warm glowing shirt and polished finish",
  ],
  accessories: [
    "wearing a polished leather watch and tasteful cufflinks",
    "carrying an artisan leather satchel with refined craftsmanship",
    "with a traditional woven shawl draped casually across one shoulder",
    "wearing thin gold-rimmed eyeglasses that add quiet intellectual elegance",
    "with handcrafted cufflinks and a minimalist metal bracelet",
    "wearing a modern pocket square with a soft glowing accent",
    "carrying a classic fountain pen with a leather-bound notebook",
    "wearing a lightweight scarf that catches warm sunlight beautifully",
    "with a subtle silver pendant and polished natural finish",
    "wearing premium suede loafers with tasteful understated style",
  ],
  narrativeDetails: [
    "The story feels aspirational and quietly cinematic, with the subject fully immersed in his environment.",
    "The image captures calm confidence and cultural authenticity in a bright glowing scene.",
    "It reads like a high-end editorial portrait with warm luminous finish and vibrant atmosphere.",
    "The scene tells a story of refined professionalism and natural ease in daylight.",
    "It feels like a premium lifestyle portrait where authenticity and brightness are central.",
    "The composition highlights a modern South Asian man living with quiet confidence in a radiant setting.",
    "The photograph evokes soft glowing warmth, realistic texture, and elegant composure.",
    "A bright storytelling portrait with honest emotion, authentic detail, and fresh light.",
    "This image blends cultural nuance, professional polish, and glowing light with no distortion.",
    "The moment is captured with vivid clarity, natural depth, and an inviting luminous quality.",
  ],
  cameras: [
...
export function generateOfflinePrompt(category: PromptCategory): string {
  ...
  do {
    ...
    isCombinationRepeated(
      getCombinationKey(subject, location, pose, lighting),
    ) &&
    attempts < maxAttempts
  );

  const weather = getRandomItem(offlineData.weather);
  const camera = getRandomItemExcluding(
    offlineData.cameras,
    diversityTracker.cameras,
  );
  const lens = getRandomItem(offlineData.lenses);
  const composition = getRandomItem(offlineData.compositions);
  const mood = getRandomItemExcluding(
    offlineData.moods,
    diversityTracker.moods,
  );
  ...
  const clothingStyle = getRandomItem(offlineData.clothingStyles);
  const accessory = getRandomItem(offlineData.accessories);
  const narrativeDetail = getRandomItem(offlineData.narrativeDetails);
  ...
  diversityTracker.clothingStyles.add(clothingStyle);
  ...
  const prompt = `${subject}, ${pose.toLowerCase()} in ${location}. ${clothingStyle}. ${accessory}. 
...
Scene Narrative: ${narrativeDetail}

Aesthetic Direction: ${categoryModifier}

Photography Quality: ${qualityTerm}. 

Critical Requirements: Natural balanced exposure with glowing light, vibrant colors, and no face distortion. Absolutely no dark, moody, underexposed, blue-toned, or grey-heavy tones. No distorted anatomy, no melted features, and no facial artifacts. Professional clarity and definition throughout. High dynamic range with rich detail preservation. Ready for premium commercial publication and premium brand representation.`;
...
export function generateOfflinePrompts(
  ...
  diversityTracker.clothingStyles.clear();
